...
#'@param effect.size.prior The prior of the effect size, where the 'Spike-slab' prior is the default prior.
...
#'@param LD.estimation he indicator variable of whether turn on the LD estimation of cross-ancestry correlations. We suggest that the LD estimation is used if using external LD matrix. 
#'@param rho.index A number between 0 and 1 specifying \eqn{\rho} of the estimated credible sets.
#'@param BF.index  A number smaller than 1 to specifying the threshold of the Bayes factor of the estimated credible models. The default setting is 10.
...
#'@param outlier.BF.index The threshold of the Bayes factor of the hypothesis testing of determining outliers, which is 1/3.2 by default. 
...
CARMAX<-function(z.list,ld.list,lambda.list=NULL,output.labels='.',label.list=NULL,LD.estimation=T,
                                ...
                                Max.Model.Dim=2e+5,all.iter=3,all.inner.iter=10,epsilon.threshold=1e-4,
                                 num.causal=10,y.var=1,tau=0.04,outlier.switch=T,outlier.BF.index=1/3.2,purity.index=0.5){
 
  credible.set.fun.improved<-function(pip,ld,true.beta=NULL,rho=0.99,purity=purity.index){
    candidate.r<-c((seq(from=purity,0.95,by=0.05)),seq(0.96,0.99,0.01))
    ...
    for(r in 1:length(candidate.r)){
      ...
      while(sum(pip[pip.order[s:length(pip.order)]])>rho){
        #  working.ld[2253,2253:2260]
        #print(s)
        true.cor.group<-as.numeric(names(which(abs(working.ld[which(pip.order[s]==as.numeric(colnames(working.ld))),])>cor.threshold)))
        cor.group<-which(abs(working.ld[which(pip.order[s]==as.numeric(colnames(working.ld))),])>cor.threshold)
        if(min(abs(working.ld[cor.group,cor.group]))<0.5){
          #names(cor.group)<-true.cor.group
          cor.group<-cor.group[order(pip[as.numeric(names(cor.group))],decreasing = T)]
          co=2
          repeat{
            if(any(abs(working.ld[cor.group[co],cor.group[co:length(cor.group)]])<0.5)){
              drop.index<- cor.group[co:length(cor.group)][which(abs(working.ld[cor.group[co],cor.group[co:length(cor.group)]])<0.5)]
              if(sum(pip[as.numeric(names(drop.index))])<pip[as.numeric(names(cor.group[co]))]){
                cor.group<-cor.group[-match(drop.index,cor.group)]
              }else{
                cor.group<-cor.group[-cor.group[co]]
              }
            }
            
            if(co==length(cor.group)){
              break
            }
            if(sum(pip[as.numeric(names(cor.group))])<rho){
              break
            }
            co=co+1
          }
        }
        cor.group<-as.numeric(names(cor.group))
  ...
  ########## Input data#########
  {
    ...
    L<-length(z.list)
    p.list<-list()
    z.vector<-matrix(NA,0,1)
    for(i in 1:L){
    #  print(nrow(z.list[[i]]))
      z.list[[i]]<-as.matrix(z.list[[i]])
      p.list[[i]]<-nrow(z.list[[i]])
      z.vector<-matrix(c(z.vector,z.list[[i]]))
      }
...
#########Module model##########
  ...
  Module.Cauchy.Shotgun<-function(z,ld.matrix,Max.Model.Dim=1e+4,input.S=NULL,lambda,label,multi.ethnic,
                                              ...
                                              C.list=NULL,prior.prob=NULL,epsilon=1e-3,inner.all.iter=10){
       {
         ...
         #z<-z.vector
         #ld.matrix<-as.matrix(ld.all)
         #tau=y.var/10;lambda=1;outlier.switch=F
         prob.list<-list()
      ...
      ###### Define marginal likelihood#########  
         ...
         if(effect.size.prior=='multi-study-cauchy'){
           tau.sample<-rgamma(1e+5,0.5,rate=0.5)
           if(multi.ethnic=='admixed'){
             
             marginal_likelihood<-function(input_S,Sigma,z,tau,p_S,y_sigma){
               used_S<-c(input_S,input_S+actual.p/2)
               margin<-Cauchy_fixed_sigma_marginal(used_S,Sigma,z,tau.sample,p_S*2,1)
               return(margin)
             }
           }
         }
         if(effect.size.prior=='multi-study-horseshoe'){
           kappa<-rbeta(1e+5,0.5,0.5)
           tau.sample<-kappa/(1-kappa)
           if(multi.ethnic=='admixed'){
         ...
         if(effect.size.prior=='multi-study-Hyper'){
           print(multi.ethnic)
           tau.sample<-rgamma(1e+5,0.5,rate=0.5)
           ...
           if(multi.ethnic=='admixed'){
             ...
             marginal_likelihood<-function(input_S,Sigma,z,tau,p_S,y_sigma){
               used_S<-c(input_S,input_S+actual.p/2)
               margin<-hyper_g_fixed_sigma_marginal(used_S,Sigma,z,tau.sample,p_S*2,1)
               return(margin)
             }
           }
           ...
           if(outlier.switch){
             if(outlier.hypo.form=='Original'){
               outlier_likelihood=marginal_likelihood
               outlier.tau=tau.sample
             }
             if(outlier.hypo.form=='New.way'){
               outlier_likelihood=marginal_likelihood
         ...
         if(effect.size.prior=='multi-study'){
           marginal_multi_study<-function(input_S,Sigma,z,tau,p_S,y_sigma){
             used_S<-c(input_S,input_S+actual.p/2)
             pp_S<-p_S*2
             Sigma_S<-Sigma[used_S,used_S]
             A=matrix(0,pp_S,pp_S)
             
             diag(A)=diag(A)+tau+y_sigma
             sub_A=diag(y_sigma,p_S)
             A[1:p_S,(p_S+1):pp_S]<-sub_A
             A[(p_S+1):pp_S,1:p_S]<-sub_A
             A_inv=ginv(A)
             det_A=det(A)
             det_S=det(Sigma_S+A_inv)
             Sigma_S_inv=ginv(Sigma_S+A_inv)
             sub_z=as.matrix(z[used_S,])
             b=c(-0.5*(log(det_A)+log(det_S))+(t(sub_z)%*%Sigma_S_inv%*%sub_z)/2)
             return(b)
           }    
             
           marginal_likelihood=marginal_multi_study
           tau.sample<-y.var/10
           
         }
         
         if(effect.size.prior=='multi-study-indep'){
           marginal_multi_study<-function(input_S,Sigma,z,tau,p_S,y_sigma){
             used_S<-c(input_S,input_S+actual.p/2)
             pp_S<-p_S*2
             Sigma_S<-Sigma[used_S,used_S]
         ...
         if(effect.size.prior=='Spike-slab'){
           ...
           if(multi.ethnic=='admixed'){
             ...
             marginal_likelihood<-function(input_S,Sigma,z,tau,p_S,y_sigma){
               used_S<-c(input_S,input_S+actual.p/2)
               ap_S<-length(used_S)
               margin<-ind_Normal_fixed_sigma_marginal(used_S,Sigma,z,tau.sample,ap_S,1)
               return(margin)
             }
           }
           if(outlier.switch){
      ...
      B<-Max.Model.Dim#<-2e+5
      stored.result.prob<-rep(0,actual.p)
      stored.bf<-0
      Sigma<-as.matrix(ld.matrix)
      
      
      ld1<-Sigma[1:p,1:p]
      ld2<-Sigma[(p+1):(2*p),(p+1):(2*p)]
      z1<-z[1:p,]
      z2<-z[(p+1):(2*p),]
   ...
   #     print(nrow(x));print(p)
    ...
    if(multi.ethnic=='admixed'){
      ridge.fun<-function(x){
        ...
        temp.Sigma[actual.test.S,actual.test.S]<-temp.ld.S
        ...
        { 
         #conditional.S<-NULL;S<-928
          ...
          if(length(working.S)!=0){
            ...
            for(i in 1){
              aa<-set.gamma.margin[[i]]-c(current.log.margin)
            ...
            for(i in 2:length(set.gamma)){
              ...
              if(length(test.S)>1){
                ...
                if(multi.ethnic=='admixed'){
                  actual.test.S<-c(test.S,test.S+actual.p/2)
              modi.ld.S<- modi.Sigma[actual.test.S,actual.test.S]
                }else{
              ...
              if(outlier.hypo.form=='New.way'){
                ...
                if(multi.ethnic=='admixed'){
              modi.Sigma[actual.test.S,actual.test.S]<-modi.ld.S
                }else{
           ...
           print(set.star)
            ...
            if(length(working.S)==num.causal){
              set.star<-set.star[-2,]
              aa<-set.star$margin-c(current.log.margin)-max(set.star$margin-c(current.log.margin))
            ...
            S<-set.gamma[[sec.sample]][set.star$gamma.set.index[[sec.sample]] ,]
                ...
                set.star<-data.frame(set.index=rep(1,3),gamma.set.index=rep(NA,3),margin=rep(NA,3))
                         aa<-set.gamma.margin[[2]]-c(current.log.margin)
                         aa<-aa-aa[which.max(aa)]
                         if(length(which(is.nan(aa)))!=0){
                           aa[which(is.nan(aa))]<-min(aa)
                         }
  ...
  for(i in 1:L){
    t0=Sys.time()
   all.C.list[[i]]<-Module.Cauchy.Shotgun(z=z.vector,ld.matrix=ld.all,epsilon=epsilon.list[[i]],multi.ethnic=multi.ethnic,
  ...
  for(g in 1:all.iter){ 
    ...
    for(i in 1:L){
      t0=Sys.time()
      all.C.list[[i]]<-Module.Cauchy.Shotgun(z=z.vector,ld.matrix=ld.all,input.conditional.S.list = all.C.list[[i]][[4]],
                                             ...
                                             outlier.switch=outlier.switch,tau=tau,multi.ethnic=multi.ethnic,
  ...
  for(i in 1:length(z.list)){
    ...
    credible.set<-credible.set.fun.improved(ance.pip,ance.ld,rho=rho.index)
    credible.model<-credible.model.fun(all.C.list[[1]][[1]][[1]],all.C.list[[1]][[1]][[2]],bayes.threshold = BF.index)
    results.list[[i]][[1]]<-ance.pip
    results.list[[i]][[2]]<-credible.set
    results.list[[i]][[3]]<-credible.model
    results.list[[i]][[4]]<-all.C.list[[1]][[4]]
